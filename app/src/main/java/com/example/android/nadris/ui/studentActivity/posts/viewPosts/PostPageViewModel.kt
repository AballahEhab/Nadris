...
class PostPageViewModel @Inject constructor(val repository: Repository) : ViewModel() {
    ...
    private val _bookMarkPostResults = MutableLiveData<Result<Boolean>>()
    val bookMarkPostResults: LiveData<Result<Boolean>> get() = _bookMarkPostResults
    ...
    fun bookMark(post: DatabasePost) {
        viewModelScope.launch {
            repository.bookMarkAPost(post).collect{ result->
                _bookMarkPostResults.value = result
            }
        }
    }
